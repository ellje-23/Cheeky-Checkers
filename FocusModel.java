...
public class FocusModel {
	...
	public void move(Location source, Location destination) {
		Deque<Integer> hand = new Deque<Integer>(); // Pieces being moved.
		int n = 1; // Distance/pieces to move.
		...
		// Remove from source and put in hand.
		for (int i = 0; i < n; i++) {
			hand.addFront(sourcePile.removeBack());
		}
		// Deposit pieces from hand to destination
		Deque<Integer> dest = getPile(destination);
		while (!hand.isEmpty()) {
			dest.addBack(hand.removeFront());
		}
		// Check for captures and reserves due to overflow
		while (dest.size() > 5) {
			int color = dest.removeFront();
